	...
	AwesomeStackTV.prototype = AwesomeStackTV.fn = {
		...
		init: function(option, undefined) {
				...
				tv_list: {
					...
					set: function(newValue) {
						if (newValue instanceof Array) {
							...
							console.info("La lista de programas ha sido actualizada.");
							_this._genSwitches();
						} else {
							console.warn("El tipo de asignación debe ser una matriz!");
						}
					}
				},
				currentLoadIndex: {
			...
			if (_this._setting.showAbout) {
				_this.log("Prepárate para jugar ...");
		...
		loadUrl: function(playUrl) {
			...
			try {
				...
				this.log("El enlace del vídeo es incorrecto.!");
				return;
			}
			this.log("Listo para reproducir vídeo:" + url);
			let canPlayM3u8 = this._stackvideo.canPlayType('application/vnd.apple.mpegurl');
			if (!canPlayM3u8 && url.match(/.*\.(m3u8).*/gi)) {
				this.log("El navegador actual no admite la reproducción m3u8!");
				this.log("Intentaré reproducir el video usando hls...");
				if (Hls.isSupported()) {
					this.log("Cargando hls...");
					this.hls.on(Hls.Events.ERROR, function(event, data) {
						// _this.log("hls:Error al cargar vídeo");
						if (data.fatal) {
							switch (data.type) {
								case Hls.ErrorTypes.NETWORK_ERROR:
									_this.log("hls:Error de red, intenta solucionarlo...");
									_this.hls.startLoad();
									break;
								case Hls.ErrorTypes.MEDIA_ERROR:
									_this.log("hls:Error multimedia, intenta solucionarlo...");
		...
		loadTVList: function(list) {
			this.log("Listo para cargar la lista de programas..");
			if (!list || typeof list == 'string' || !list.length) {
				this.log("El formato de datos de la lista de programas es incorrecto.!");
				return false;
			}
			this._setting.tv_list = list;
			if (!this._setting.showAbout && this._setting.autoPlayFirst && this._setting.tv_list[0]) {
				this.log("Cargar automáticamente el primer programa.");
		...
		_initTV: function() {
			...
			switchHead.innerText = "lista de programas";
			let switchSearch = document.createElement("div");
			let input = document.createElement("input");
			input.placeholder = "buscar programas";
			...
			video.addEventListener("loadstart", function() {
				_this.log("Empezar a cargar...");
			});
			video.addEventListener("loadeddata", function() {
				_this._stackvideo.removeAttribute("poster");
				_this.log("Cargando duro...");
			});
			video.addEventListener("canplaythrough", function() {
				_this.log("empezar a jugar...");
			...
			video.addEventListener("error", function() {
				_this.log("Hay un problema con la reproducción.");
			});
			...
			_this.log("~Stack TV Cargando completada");
			...
			function lazyLoad() {
				...
				if (lastSwitch != undefined && _this._setting.currentLoadIndex > 0 && _this._setting
					...
					console.info("Cargar más listados de programas");
					_this._genSwitches();
